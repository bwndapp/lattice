...
export default function SynthWindow({ project, patternId, channelId, order, front, onUpdateProject }) {
  ...
  return (
    ...
    >
      ...
      >
        {/* what you called it; what it is, unless the name says so already; where it lives */}
        <h2 id={`sw-title-${channelId}`} className="sw-title">{ch.name}</h2>
        {!ch.name.toLowerCase().includes(spec.label.toLowerCase()) && <span className="sw-kind">{spec.label}</span>}
        {/* a pattern made for one instrument carries its name: no need to say it twice */}
        {pattern.name.trim().toLowerCase() !== ch.name.trim().toLowerCase() && <span className="sw-where">{pattern.name}</span>}
