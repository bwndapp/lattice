...
import { BANKS, DRUM_SOUNDS, SYNTH_SOUNDS, makeChannel, makePattern, midiToNote, newId, noteToMidi, songLength, stepCount } from './project'
...
/**
 * The channel rack for one pattern: a row per channel, a step grid per row. Everything
 * here edits the project, which regenerates the code.
 */
export default function Rack({ project, currentPatternId, onSelectPattern, onUpdateProject, transport, started, playMode, onPlayMode }) {
  ...
  const setShape = (patch) => update((pat) => {
  ...
  const addChannel = (kind) => update((pat) => {
    const count = pat.channels.filter((c) => c.kind === kind).length + 1
    const defaults = { drum: { name: `drum ${count}`, sound: DRUM_SOUNDS[(count - 1) % DRUM_SOUNDS.length] }, synth: { name: `synth ${count}` }, code: { name: `code ${count}` } }
    pat.channels.push(makeChannel(kind, defaults[kind]))
  })

  const newPattern = () => onUpdateProject((p) => {
    const np = makePattern(`pattern ${p.patterns.length + 1}`, { channels: [makeChannel('drum', { name: 'kick', sound: 'bd' })] })
    p.patterns.push(np)
    ...
    <section className="rack" aria-label="Channel rack">
      <div className="rack-head">
        ...
        <label className="rack-field">
          ...
          <select className="select" value={pattern.bars} onChange={(e) => setShape({ bars: Number(e.target.value) })} aria-label="Bars in this pattern">
            {[1, 2, 4, 8].map((b) => <option key={b} value={b}>{b}</option>)}
          </select>
        </label>
        <label className="rack-field">
          <span className="syn">steps/bar</span>
          <select className="select" value={pattern.stepsPerBar} onChange={(e) => setShape({ stepsPerBar: Number(e.target.value) })} aria-label="Steps per bar">
      ...
      <div className="rack-body" ref={gridsRef} style={{ '--n': n, '--spb': stepsPerBeat }}>
        {pattern.channels.length === 0 && <p className="rack-hint">Add a channel to start: a drum, a synth, or a line of Strudel code.</p>}
        {pattern.channels.map((ch) => (
          <div key={ch.id} className={`ch ${ch.mute ? 'muted' : ''}`}>
            ...
            <div className="ch-more">
              ...
              <button className="btn ghost danger ch-btn" title="Delete channel" aria-label={`Delete ${ch.name}`} onClick={() => update((pat) => { pat.channels = pat.channels.filter((c) => c.id !== ch.id) })}>del</button>
            </div>
          </div>
        ))}
        <div className="rack-add">
          <button className="btn" onClick={() => addChannel('drum')}>+ drum</button>
          <button className="btn" onClick={() => addChannel('synth')}>+ synth</button>
          <button className="btn" onClick={() => addChannel('code')}>+ code</button>
        </div>
      </div>
